      ...
      <div className="max-w-[500px] w-full space-y-8">
        <div className="text-center">
          ...
          <p className="mt-5 text-sm text-gray-600">
            {isAdminLogin ? 'Connectez-vous pour accéder au tableau de bord' : "Ce questionnaire a pour objectif de recueillir des informations sur les budgets prévus par les dirigeants d’entreprises, professions libérales de santé (médecins, kinésithérapeutes, sage-femmes, infirmiers, etc.), juridique (avocats, notaires, etc.) pour leur protection sociale et leur retraite. Les réponses permettront de mieux comprendre les besoins et attentes en matière de prévoyance, santé et retraite."}
          </p>
        </div>
          ...
          <div className="rounded-md shadow-sm -space-y-px">
            ...
            <div className="mt-4">
              ...
              <div className="flex">
                ...
                <input id="phone" name="phone" type="tel" value={phone} onChange={e => setPhone(e.target.value)} required className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-r-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm" placeholder="Entrez votre numéro de téléphone" />
              </div>
            </div>
          </div>
          <div className="flex items-center">
            <input id="admin-login" name="admin-login" type="checkbox" checked={isAdminLogin} onChange={() => setIsAdminLogin(!isAdminLogin)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
            <label htmlFor="admin-login" className="ml-2 block text-sm text-gray-900">
              Connexion administrateur
            </label>
          </div>
          <div>
            <button type="submit" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-orange-500 hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
              <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                <ShieldIcon className="h-5 w-5 text-blue-500 group-hover:text-blue-400" />
              </span>
